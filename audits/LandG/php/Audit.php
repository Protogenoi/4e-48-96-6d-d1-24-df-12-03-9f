...
grade:=GRADE,
 ...
 date_submitted=CURRENT_TIMESTAMP");
    ...
    if ($query >= '1') {
        header('Location: ../Audit.php?RETURN=ADDED&grade=' . $grade . '&TotalCorrect=' . $totalCorrect . '&TotalPercent=' . $percentage . '&TotalGrade=' . $percentage2);
        die;
    } else {
        header('Location: ../Audit.php?RETURN=AuditEditFailed&Error');
        die;
    }
}
}
