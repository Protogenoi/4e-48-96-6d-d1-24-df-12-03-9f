...
<br>
    ...
    <?php 
    $query = $pdo->prepare("select count(audit_id) AS audit_id from RoyalLondon_Audit where grade ='SAVED' and added_by =:hello");
    $query->bindParam(':hello', $hello_name, PDO::PARAM_STR, 12);
    $query->execute();
    if ($query->rowCount()>0) {
        while ($result=$query->fetch(PDO::FETCH_ASSOC)){
            $savedcount = $result['audit_id'];
            if ($savedcount >=1){
                ...
                $query = $pdo->prepare("SELECT plan_number, audit_id, added_date, closer, added_by, grade, updated_by, updated_date from RoyalLondon_Audit where added_by=:hello and added_date between DATE_ADD(CURDATE(), INTERVAL 1-DAYOFWEEK(CURDATE()) DAY) AND DATE_ADD(CURDATE(), INTERVAL 7-DAYOFWEEK(CURDATE()) DAY) or updated_date between DATE_ADD(CURDATE(), INTERVAL 1-DAYOFWEEK(CURDATE()) DAY) AND DATE_ADD(CURDATE(), INTERVAL 7-DAYOFWEEK(CURDATE()) DAY) AND updated_by =:hello ORDER BY added_date DESC");
                $query->bindParam(':hello', $hello_name, PDO::PARAM_STR, 12);                
                $query->execute();
                $i=0;
                if ($query->rowCount()>0) {
                    ...
                    while ($result=$query->fetch(PDO::FETCH_ASSOC)){
                        ...
                        switch( $result['grade'] ) {
                            ...
                            default:
                                ...
                                echo '<tr class='.$class.'>';
                                echo "<td>".$result['audit_id']."</td>";
                                echo "<td>".$result['plan_number']."</td>";
