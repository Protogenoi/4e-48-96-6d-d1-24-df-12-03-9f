...
if ($duperaw->num_rows >= 1) {
    ...
    $query = $pdo->prepare("UPDATE google_dev set tracking_id=:trackingidhold, api=:apihold, updated_by=:userholder WHERE id =:iddupe");
        ...
        $query->execute()or die(print_r($query->errorInfo(), true));
                    ...
                    if(isset($fferror)) {
    if($fferror=='0') {
    header('Location: ../Admindash.php?Google=y&google=updated'); die;
    }
                    }
    }
    ...
    $query = $pdo->prepare("INSERT INTO google_dev set tracking_id=:trackingidhold, api=:apihold, added_by=:userholder");
        ...
        $query->execute()or die(print_r($query->errorInfo(), true));
                    ...
                    if(isset($fferror)) {
    if($fferror=='0') {
    header('Location: ../Admindash.php?Google=y&google=y'); die;
    }
                    }
}
                ...
                if(isset($fferror)) {
    if($fferror=='0') {
    header('Location: ../Admindash.php?Google=y&google=failed'); die;
    }
                }
}
